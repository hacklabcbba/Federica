...
object TransversalAreaSnippet extends SortableSnippet[TransversalArea] {
  ...
  def renderViewFrontEnd: CssSel = {
    for {
      ...
      "data-name=events" #> EventSnippet.relatedEvents(area.name.get, Empty, Empty, Empty, Empty, Empty, Empty, Empty) &
      "data-name=posts" #> BlogSnippet.relatedPosts(area.name.get, Empty, Empty, Empty, Empty, Empty, Empty, Empty) &
      "data-name=calls" #> CallSnippet.relatedCalls(area.name.get, Empty, Empty, Empty, Empty, Empty, Empty, Empty)
    }
  }
