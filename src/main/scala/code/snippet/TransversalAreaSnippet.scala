...
object TransversalAreaSnippet extends SortableSnippet[TransversalArea] {
  ...
  def renderViewFrontEnd: CssSel = {
    for {
      ...
      "data-name=events" #> EventSnippet.relatedEvents(area.name.get, Empty, Empty, Empty, Empty, Full(area), Empty, Empty) &
      "data-name=posts" #> BlogSnippet.relatedPosts(area.name.get, Empty, Empty, Empty, Empty, Full(area), Empty, Empty) &
